#ifndef MORSECODE_HPP
#define MORSECODE_HPP

#include "Coding.hpp"
#include <iostream>
#include <map>

using namespace std;

class MorseCode : public Coding
{
private:
    const string _letterSpaces = "   ";
    const string _wordSpaces = "       ";

    map<char, string> _translation = {
        {'a', ". _"},
        {'b', "_ . . ."},
        {'c', "_ . _ ."},
        {'d', "_ . ."},

        {'e', "."},
        {'f', ". . _ ."},
        {'g', "_ _ ."},
        {'h', ". . . ."},

        {'i', ". ."},
        {'j', ". _ _ _"},
        {'k', "_ . _"},
        {'l', ". _ . ."},

        {'m', "_ _"},
        {'n', "_ ."},
        {'o', "_ _ _"},
        {'p', ". _ _ ."},

        {'q', "_ _ . _"},
        {'r', ". _ ."},
        {'s', ". . ."},
        {'t', "_"},

        {'u', ". . _"},
        {'v', ". . . _"},
        {'w', ". _ _"},

        {'x', "_ . . _"},
        {'y', "_ . _ _"},
        {'z', "_ _ . ."},

        {'0', "_ _ _ _ _"},
        {'1', ". _ _ _ _"},
        {'2', ". . _ _ _"},
        {'3', ". . . _ _"},
        {'4', ". . . . _"},

        {'5', ". . . . ."},
        {'6', "_ . . . ."},
        {'7', "_ _ . . ."},
        {'8', "_ _ _ . ."},
        {'9', "_ _ _ _ ."},

        {'.', ". _ . _ . _"},
        {',', "_ _ . . _ _"},
        {'?', ". . _ _ . ."},
        {'!', "_ . _ . _ _"},

        {':', "_ _ _ . . ."},
        {';', "_ . _ . _ ."},
        {'"', ". _ . . _ ."},
        {'/',  "_ . . _ ."},
        {'\\', "_ . . _ ."},

        {'(', "_ . _ _ ."},
        {')', "_ . _ _ . _"},
        {'=', "_ . . . _"},
        {'+', ". _ . _ ."},
        {'-', "_ . . . . _"}
    };

    string EncodeWord(string);
    string DecodeWord(string);

    void CheckInput(string&);
public:
    string Encode(string) override;
    string Decode(string) override;

    void EncodeFile(string, string) override;
    void DecodeFile(string, string) override;
};

#endif //MORSECODE_HPP